...
function App() {
  ...
  const handleLogin = (e) => {
    e.preventDefault();  //移除原生submit事件
    axios.post(`${URL}/v2/admin/signin`, account)
      .then((res) => {
        const { token, expired } = res.data;
        console.log(token, expired);
        document.cookie = `ajiToken=${token}; expires=${new Date(expired)}`;
  ...
  return (
    ...
    {isAuth ? (    <div className="container py-5">
      <div className="row">
        <div className="col-6">
          <button onClick={checkUserLogin} className='btn btn-success mb-5' type="button">驗證登入</button>
          <h2>產品列表</h2>
